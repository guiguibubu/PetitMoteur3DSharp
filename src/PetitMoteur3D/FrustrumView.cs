...
internal sealed class FrustrumView
{
    ...
    public float AspectRatio
    {
        ...
        set
        {
            ...
            {
                _aspectRatio = value; UpdateMatProj();
            }
        }
    }
    ...
    public FrustrumView(float fieldOfView, float aspectRatio, float nearPlaneDistance, float farPlaneDistance, bool isOrthographic = false)
    {
    ...
    private void UpdateMatProj()
    {
        ...
        {
            SizeF farPlaneSize = GetOrthographicFarPlaneSize(_fieldOfView, _aspectRatio, _nearPlaneDistance, _farPlaneDistance);
    ...
    private static SizeF GetOrthographicFarPlaneSize(float fieldOfView, float aspectRatio, float nearPlaneDistance, float farPlaneDistance)
    {
        float nbPortionMax = 3f;
        float nbPortion = 2f;
        float planMedian = (nbPortion * farPlaneDistance + (nbPortionMax - nbPortion) * nearPlaneDistance) / nbPortionMax;
        float heightFarPlane = 2 * planMedian * (float)System.Math.Tan(fieldOfView / 2.0f);
        float widthFarPlane = aspectRatio * heightFarPlane;
        return new SizeF(widthFarPlane, heightFarPlane);
    }
}
