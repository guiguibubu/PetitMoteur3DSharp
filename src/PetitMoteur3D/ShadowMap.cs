...
internal class ShadowMap
{
    ...
    private unsafe void InitDepthBuffer(int width, int height)
    {
        ...
        _depthTexture = _textureManager.GetOrCreateTexture("ShadowMap_DepthTexture", depthTextureDesc, (TextureBuilder builder) => builder.WithShaderRessourceView(shaderResourceViewDesc).WithDepthStencilView(descDSView));
    }
