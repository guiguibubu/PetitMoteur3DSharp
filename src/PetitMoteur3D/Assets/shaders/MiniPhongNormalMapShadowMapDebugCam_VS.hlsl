...
cbuffer objectShadowBuffer : register(b3)
{
    float4x4 matWorldViewProjLight; // Matrice WVP pour lumière
}

cbuffer frameDebugBuffer : register(b4)
{
    bool isDebugCamUsed; // scene drawn or debug cam
}

VS_Sortie MiniPhongNormalMapShadowMapDebugCamVS(float4 Pos : POSITION, float3 Normale : NORMAL, float2 coordTex : TEXCOORD, float3 Tangent : TANGENT)
{
    ...
    if (isDebugCamUsed)
    {
        sortie.gameSpacePos = mul(Pos, matWorldViewProj);
    }
